...
| 2 | `run_parameter_estimation.py` | trained networks (`models/iat_{ddm,oum}.keras`), `iat_results_{ddm,oum}.csv`, recovery diagnostics (Appendix C3/C4) |
...
`run_parameter_estimation.py` saves the trained DDM/OUM networks to `models/`.
...
- Offline training: fixed bank of 64,000 simulations (+ 1,000 validation)
- Epochs: 100
- Summary network: SetTransformer (20 dimensions)
- Inference network: CouplingFlow (affine, default)
- Initial learning rate: 5e-4
- Posterior samples per participant: 3,000
- Recovery diagnostics: 500 simulated data sets
- Out-of-distribution participants whose posterior median is non-finite or > 100 in any unbounded parameter (drift, error NDT, *k*) are excluded from aggregate statistics
...
- `iat_data/estimates/iat_model_comparison_results.csv` — posterior model probabilities per person (large, not tracked)
- `iat_data/estimates/iat_demographics.csv` — age, gender, education, country (large, not tracked)
...
| *a*₁, *a*₂ (boundary, congruent/incongruent) | 8·σ(𝒩(−1.6, 1.0)) | 8·σ(𝒩(−0.25, 1.0)) |
| *τ*_c (correct NDT) | U(0.1, 1.0) | U(0.1, 1.0) |
| *τ*_e (error NDT) | Γ(2.0, 0.3) | Γ(2.0, 0.3) |
| *k* (self-excitation) | — | Γ(8.0, 0.5) |
