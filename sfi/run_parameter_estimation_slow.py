"""Run parameter estimation (DDM + OUM) for slow tasks.

Trains BayesFlow inference networks for both DDM and OUM, runs diagnostics,
then estimates posterior parameters for all participants across 9 slow tasks.
Saves .npy estimate files and Appendix A4 diagnostics figures.

Usage:
    python run_parameter_estimation_slow.py
"""
...
for fig_name, fig in figures.items():
    path = f"{FIGURES_DIR}figureA4_recovery_slow_ddm_{fig_name}.pdf"
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved {path}")

# Per-task estimation + PPC
print(f"\nEstimating DDM posteriors for {len(FILES)} slow tasks...")
for file in FILES:
...
for fig_name, fig in figures.items():
    path = f"{FIGURES_DIR}figureA4_recovery_slow_oum_{fig_name}.pdf"
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved {path}")

# Per-task estimation + PPC
print(f"\nEstimating OUM posteriors for {len(FILES)} slow tasks...")
for file in FILES:
    task_name = os.path.basename(file).replace(".txt", "")
    print(f"\n  Task: {task_name}")
...
print("\n" + "=" * 60)
print("Slow task parameter estimation complete.")
print("=" * 60)
