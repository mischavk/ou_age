"""Run model comparison (DDM vs OUM) for fast tasks.

Trains a BayesFlow ModelComparisonApproximator, validates on simulated data,
then applies to empirical fast task data. Saves Figure 2 and Appendix A1/A2.

Usage:
    python run_model_comparison_fast.py
"""
...
f.savefig(f"{FIGURES_DIR}figureA1_calibration_fast.pdf", bbox_inches="tight")
plt.close(f)
print(f"  Saved {FIGURES_DIR}figureA1_calibration_fast.pdf")
...
f.savefig(f"{FIGURES_DIR}figureA2_confusion_matrix_fast.pdf", bbox_inches="tight")
plt.close(f)
print(f"  Saved {FIGURES_DIR}figureA2_confusion_matrix_fast.pdf")
...
print(f"\nApplying to {len(files)} empirical fast tasks...")
print("Model probabilities for DDM vs. OUM on fast tasks:")
...
for ax, file in zip(axes, files):
    ...
    # Bayes factor
    pmp_oum = np.array(pred_models_empirical[:, 1]).astype(float)
    pmp_oum = pmp_oum[~np.isnan(pmp_oum)]
    epsilon = 1e-7
    pmp_oum_clipped = np.clip(pmp_oum, epsilon, 1 - epsilon)
...
plt.tight_layout()
fig.savefig(f"{FIGURES_DIR}figure2_model_comparison_fast.pdf", bbox_inches="tight")
plt.close(fig)
print(f"\nFigure 2 (fast) saved to {FIGURES_DIR}figure2_model_comparison_fast.pdf")
print("Done.")
